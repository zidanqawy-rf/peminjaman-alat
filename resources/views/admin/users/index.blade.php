<x-admin-layout title="Kelola User">
    <div class="md:ml-64 min-h-screen bg-gray-100">
        <div class="p-6" x-data="{ openAdd: false, openEdit: false, editUserId: null, editUserData: null, deleteUserId: null, openDelete: false, openImport: false, importFile: null, importLoading: false }">
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        ...
                                        @elseif ($user->isPetugas())
                                            <span class="px-3 py-1 bg-emerald-100 text-emerald-800 rounded-full text-xs font-semibold">Petugas</span>
                                        @else
                                            <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold">User</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm flex items-center space-x-3">
                                        <button
                                            @click="editUserId = {{ $user->id }}; editUserData = {name: '{{ $user->name }}', email: '{{ $user->email }}', role: '{{ $user->role }}', kelas: '{{ $user->kelas }}', jurusan: '{{ $user->jurusan }}', nisn: '{{ $user->nisn }}'}; openEdit = true"
                ...
                <form action="{{ route('admin.register') }}" method="POST" class="space-y-4">
                    ...
                    <div class="flex items-center space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <button type="button" @click="openAdd = false"
                            class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition-colors font-medium">Batal</button>
                    </div>
                </form>
            </x-admin-modal>
                    ...
                    <form :action="`/admin/users/${editUserId}`" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="role" id="edit_role"
                                ...
                                <option value="petugas" :selected="editUserData?.role === 'petugas'">Petugas</option>
                            </select>
                        </div>

                        <div>
                            <label for="edit_password" class="block text-sm font-medium text-gray-800 mb-1">Password <span class="text-gray-400 text-xs">(Kosongkan jika tidak ingin mengubah)</span></label>
                            <input type="password" name="password" id="edit_password"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>

                        <div>
                            <label for="edit_password_confirmation" class="block text-sm font-medium text-gray-800 mb-1">Konfirmasi Password</label>
            ...
            <div x-show="openDelete" @click="openDelete = false"
                ...
                <div @click.stop class="bg-white rounded-lg shadow-xl max-w-sm w-full">
                    ...
                    <div class="p-6">
                        <p class="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus user ini? Tindakan ini tidak dapat diubah.</p>
                        <div class="flex items-center space-x-3">
                            <template x-if="deleteUserId">
                                <form :action="`/admin/users/${deleteUserId}`" method="POST" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium">Hapus</button>
                                </form>
                            </template>
                            <button type="button" @click="openDelete = false"
                                class="flex-1 px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition-colors font-medium">Batal</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="px-6 py-4">
                        <form id="importForm" class="space-y-4">
                            ...
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                ...
                                <p class="text-sm text-blue-800 mb-2">File harus memiliki kolom dengan urutan:</p>
                                <div class="grid grid-cols-2 gap-2 text-sm text-blue-800">
                                    <div>1. <strong>Nama</strong></div>
                                    <div>2. <strong>Email</strong></div>
                                    <div>3. <strong>Password</strong> (min. 8 karakter)</div>
                                    <div>4. <strong>Role</strong> (user/petugas/admin)</div>
                                    <div>5. <strong>Kelas</strong></div>
                                    <div>6. <strong>NISN</strong></div>
                                    <div>7. <strong>Jurusan</strong></div>
                                </div>
                            </div>

                            <div class="space-y-2">
                                <label class="block text-sm font-medium text-gray-800">Pilih File Excel/CSV</label>
                                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-500 transition-colors" id="dropZone">
                                    <svg class="w-12 h-12 mx-auto text-gray-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                    </svg>
                                    <p class="text-gray-600">Drag and drop file di sini atau <span class="text-blue-600 font-medium">klik untuk browse</span></p>
                                    <p class="text-sm text-gray-500 mt-1">Format: CSV, XLSX, XLS</p>
                                    <input type="file" id="fileInput" name="file" accept=".csv,.xlsx,.xls,.txt" class="hidden">
                                </div>
                            ...
                            <div id="importErrors" class="space-y-2" style="display: none;">
                                ...
                                <div id="errorsList" class="bg-red-50 border border-red-200 rounded p-3 text-sm text-red-700"></div>
                            </div>
                        </form>
                    </div>

                    <div class="px-6 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-end gap-3 sticky bottom-0">
            ...
            <script>
                ...
                document.getElementById('searchInput').addEventListener('keyup', function(e) {
                    ...
                    tableRows.forEach(row => {
                        ...
                        const jurusan = row.querySelector('td:nth-child(6)')?.textContent.toLowerCase() || '';
                        const nisn = row.querySelector('td:nth-child(7)')?.textContent.toLowerCase() || '';
                        const matches = name.includes(searchTerm) || email.includes(searchTerm) || role.includes(searchTerm) || jurusan.includes(searchTerm) || nisn.includes(searchTerm);
                        row.style.display = matches ? '' : 'none';
                        if (matches) visibleCount++;
                    });
                    document.getElementById('recordsInfo').textContent = visibleCount === 0
                        ? 'Tidak ada data yang sesuai dengan pencarian'
                        : `Menampilkan ${visibleCount} dari {{ count($users) }} data user`;
                });
                ...
                dropZone.addEventListener('drop', (e) => {
                    ...
                    if (e.dataTransfer.files.length > 0) { fileInput.files = e.dataTransfer.files; updateFilePreview(); }
                });
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            if (data.errors && data.errors.length > 0) {
                                document.getElementById('errorsList').innerHTML = data.errors.map(err => `<div>• ${err}</div>`).join('');
