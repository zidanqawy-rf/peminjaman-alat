                ...
                <form action="{{ route('admin.alat.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            Kategori tidak ada?
                            <a href="{{ route('admin.kategori.index') }}" target="_blank" class="text-blue-600 hover:underline">Tambah kategori baru</a>
                        </p>
                        @error('kategori')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div x-show="openImport" @click="openImport = false"
                ...
                <div @click.stop class="bg-white rounded-lg shadow-xl max-w-md w-full">
                    ...
                    <form action="{{ route('admin.alat.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
                        ...
                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-800 mb-2">Pilih File Excel <span class="text-red-500">*</span></label>
                            <input type="file" name="file" accept=".xlsx,.xls" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <p class="text-xs text-gray-500 mt-1">Format: .xlsx atau .xls (Max: 5MB)</p>
                        </div>
                        <div class="flex items-center space-x-3">
                            <button type="submit" class="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium">Import Data</button>
            ...
            <script>
                document.getElementById('searchInput').addEventListener('keyup', function(e) {
                    ...
                    tableRows.forEach(row => {
                        ...
                        const matches = nama.includes(searchTerm) || kategori.includes(searchTerm) || status.includes(searchTerm);
                        row.style.display = matches ? '' : 'none';
                        if (matches) visibleCount++;
                    });
                    document.getElementById('recordsInfo').textContent = visibleCount === 0
                        ? 'Tidak ada data yang sesuai dengan pencarian'
                        : `Menampilkan ${visibleCount} dari {{ count($alats) }} data alat`;
                });
