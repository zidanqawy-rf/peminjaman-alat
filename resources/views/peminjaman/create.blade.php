<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">Form Peminjaman Alat</h2>
    </x-slot>
    ...
    <style>
        ...
        .pm-steps{display:flex;align-items:center}.pm-step{display:flex;align-items:center;gap:10px}
        ...
        .pm-pill:hover{border-color:#2563eb;color:#2563eb}.pm-pill.on{background:#2563eb;border-color:#2563eb;color:#fff}
        .pm-list{max-height:360px;overflow-y:auto}
        .pm-list::-webkit-scrollbar{width:5px}.pm-list::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:4px}
        .pm-list-item{display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid #f1f5f9;transition:background .15s}
        .pm-list-item:last-child{border-bottom:none}.pm-list-item:hover{background:#f8fafc}.pm-list-item.added{background:#eff6ff}
        ...
        .pm-stok-ok{background:#dcfce7;color:#166534}.pm-stok-low{background:#fef9c3;color:#854d0e}.pm-stok-out{background:#fee2e2;color:#991b1b}
        .pm-add-btn{padding:8px 16px;border-radius:10px;border:none;font-family:inherit;font-size:13px;font-weight:600;cursor:pointer;transition:all .15s;flex-shrink:0}
        .pm-add-btn-add{background:#2563eb;color:#fff}.pm-add-btn-add:hover{background:#1e40af}
        .pm-add-btn-remove{background:#fee2e2;color:#ef4444}.pm-add-btn-remove:hover{background:#fecaca}
        ...
        .pm-ci-qbtn{width:24px;height:24px;border-radius:6px;border:none;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;cursor:pointer;flex-shrink:0}
        .pm-ci-qbtn-m{background:#e2e8f0;color:#374151}.pm-ci-qbtn-m:hover{background:#cbd5e1}
        .pm-ci-qbtn-p{background:#2563eb;color:#fff}.pm-ci-qbtn-p:hover{background:#1e40af}
        ...
        .pm-upload:hover{border-color:#2563eb;background:#eff6ff}.pm-upload.done{border-color:#10b981;background:#f0fdf4}
        ...
        .pm-btn-submit:hover:not([disabled]){background:#1e40af;transform:translateY(-1px)}.pm-btn-submit[disabled]{background:#94a3b8;cursor:not-allowed}
    ...
    <div id="pm-wrap" class="py-8">
        <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="pm-card mb-6 p-6">
                <div class="pm-steps">
                    <div class="pm-step"><div class="pm-circle-on">1</div><span style="font-size:13px;font-weight:600;color:#2563eb;">Pilih Alat</span></div>
                    <div class="pm-line"></div>
                    <div class="pm-step"><div class="pm-circle-off">2</div><span style="font-size:13px;font-weight:500;color:#94a3b8;">Detail</span></div>
                    <div class="pm-line"></div>
                    <div class="pm-step"><div class="pm-circle-off">3</div><span style="font-size:13px;font-weight:500;color:#94a3b8;">Konfirmasi</span></div>
                </div>
            </div>
                ...
                <div style="display:grid;grid-template-columns:1fr 320px;gap:18px;align-items:start;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:16px;">
                        ...
                        <div class="pm-card p-6">
                            <div class="pm-sec">Pilih Alat (Bisa Lebih dari 1 Jenis)</div>
                            <div class="pm-selector">
                                ...
                                <div class="pm-list" id="pmList">
                                    ...
                                    @php
                                        ...
                                        $map     = ['laptop'=>'💻','komputer'=>'🖥️','proyektor'=>'📽️','kamera'=>'📷','tripod'=>'🎬','mic'=>'🎙️','speaker'=>'🔊','drone'=>'🚁','printer'=>'🖨️','tablet'=>'📱','monitor'=>'🖥️','keyboard'=>'⌨️'];
                                        $ic = '🔧'; foreach($map as $k=>$v){if(str_contains(strtolower($item->nama),$k)){$ic=$v;break;}}
                                    @endphp
                                    <div class="pm-list-item"
                                         data-id="{{ $item->id }}" data-nama="{{ $item->nama }}"
                                         data-stok="{{ $item->jumlah }}" data-cat="{{ $catName }}"
                                         data-icon="{{ $ic }}"
                                         data-img="{{ $item->gambar ? asset('storage/'.$item->gambar) : '' }}"
                                         data-avail="{{ $avail ? '1' : '0' }}">
                                        @if($item->gambar)
                                            <img class="pm-item-img" src="{{ asset('storage/'.$item->gambar) }}" alt="{{ $item->nama }}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                                        ...
                                        <div class="pm-item-info">
                                            ...
                                            @if($catName)<div class="pm-item-meta">{{ ucfirst($catName) }}</div>@endif
                                            <span class="pm-item-stok {{ $sc }}">{{ $avail ? '✓ '.$item->jumlah.' tersedia' : 'Stok habis' }}</span>
                                        </div>
                                        <button type="button" id="btn{{ $item->id }}"
                                                class="pm-add-btn {{ $avail ? 'pm-add-btn-add' : 'pm-add-btn-dis' }}"
                                                {{ !$avail ? 'disabled' : '' }}
                                                onclick="toggleItem('{{ $item->id }}')">+ Tambah</button>
                                    </div>
                                    @endforeach
                                    <div class="pm-no-result" id="pmNoResult" style="display:none;">🔍 Alat tidak ditemukan</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pm-card p-6">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                                <div>
                                    <label class="pm-label" for="tanggal_pinjam">Tanggal Mulai <span style="color:#ef4444">*</span></label>
                                    <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" required class="pm-input"
                                           value="{{ old('tanggal_pinjam', date('Y-m-d')) }}" min="{{ date('Y-m-d') }}">
                                </div>
                                <div>
                                    <label class="pm-label" for="tanggal_kembali">Tanggal Kembali <span style="color:#ef4444">*</span></label>
                                    <input type="date" name="tanggal_kembali" id="tanggal_kembali" required class="pm-input" value="{{ old('tanggal_kembali') }}">
                                </div>
                            </div>
                            <div style="margin-top:14px;">
                                <label class="pm-label">Keperluan <span style="font-weight:400;color:#64748b;">(Opsional)</span></label>
                                <textarea name="keperluan" rows="2" class="pm-input" style="resize:vertical;" placeholder="Contoh: Praktikum Lab Fisika">{{ old('keperluan') }}</textarea>
                            </div>
                            <div style="margin-top:14px;">
                                <label class="pm-label">Surat Peminjaman <span style="font-weight:400;color:#64748b;">(Opsional)</span></label>
                                <div class="pm-upload" id="pmUploadZone" onclick="document.getElementById('surat_peminjaman').click()">
                                    ...
                                    <input id="surat_peminjaman" name="surat_peminjaman" type="file" style="display:none;" accept=".pdf,.jpg,.jpeg,.png">
                                </div>
                            </div>
                            <div style="margin-top:14px;">
                        ...
                        <div style="display:flex;justify-content:flex-end;gap:10px;">
                            ...
                            <button type="submit" id="pmSubmit" class="pm-btn-submit" disabled>✓ Ajukan Peminjaman</button>
                        </div>
                    </div>
                    ...
                    <div class="pm-cart-sticky">
                        <div class="pm-card p-5">
                            ...
                            <div id="pmCartSummary" style="display:none;">
                                ...
                                <button type="button" class="pm-cart-clear" onclick="clearAll()">🗑️ Kosongkan</button>
                            </div>
                        </div>
    ...
    @php
    $alatJs = $alat->map(fn($a) => [
        'id'    => (string)$a->id,
        'nama'  => $a->nama,
        'stok'  => (int)$a->jumlah,
        'cat'   => is_string($a->kategori) ? $a->kategori : ($a->kategori?->nama ?? ''),
    ...
    function addItem(id) {
        var d = ALAT_DATA.find(function(a){ return a.id === id; });
        if (!d || !d.avail) return;
        CART[id] = { id: d.id, nama: d.nama, stok: d.stok, qty: 1, img: d.img };
        var btn = document.getElementById('btn' + id);
        if (btn) { btn.textContent = '✓ Ditambah'; btn.className = 'pm-add-btn pm-add-btn-remove'; }
        var row = btn && btn.closest('.pm-list-item');
    ...
    function removeItem(id) {
        ...
        var btn = document.getElementById('btn' + id);
        if (btn) {
            var row   = btn.closest('.pm-list-item');
            var avail = row && row.getAttribute('data-avail') === '1';
            btn.textContent = '+ Tambah';
            btn.className   = 'pm-add-btn ' + (avail ? 'pm-add-btn-add' : 'pm-add-btn-dis');
    ...
    function clearAll() {
        Object.keys(CART).forEach(function(id){ removeItem(id); });
    }
    ...
    function renderCart() {
        ...
        if (ids.length === 0) {
            empty.style.display='block'; list.innerHTML=''; summ.style.display='none'; submit.disabled=true; return;
        }
        empty.style.display='none'; summ.style.display='block'; submit.disabled=false;
        var html='', total=0;
        ids.forEach(function(id){
            var it=CART[id]; total+=it.qty;
            var thumb=it.img?'<img class="pm-ci-img" src="'+it.img+'" alt="" onerror="this.style.display=\'none\'">':'<div class="pm-ci-img-ph">🔧</div>';
            html+='<div class="pm-cart-item">'+thumb+'<div class="pm-ci-info"><div class="pm-ci-name">'+it.nama+'</div><div style="font-size:10px;color:#94a3b8">Maks '+it.stok+' unit</div></div><div class="pm-ci-qty-wrap"><button type="button" class="pm-ci-qbtn pm-ci-qbtn-m" onclick="changeQty(\''+id+'\',-1)">−</button><span class="pm-ci-qty-num">'+it.qty+'</span><button type="button" class="pm-ci-qbtn pm-ci-qbtn-p" onclick="changeQty(\''+id+'\',1)">+</button></div><button type="button" class="pm-ci-rm" onclick="removeItem(\''+id+'\')" title="Hapus">✕</button></div>';
        });
        list.innerHTML=html;
    ...
    function syncHidden() {
        ...
        Object.keys(CART).forEach(function(id, idx) {
            var it = CART[id];
            var a  = document.createElement('input'); a.type='hidden'; a.name='items['+idx+'][alat_id]'; a.value=it.id; container.appendChild(a);
            var b  = document.createElement('input'); b.type='hidden'; b.name='items['+idx+'][jumlah]';  b.value=it.qty; container.appendChild(b);
        });
    }

    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('pmSearch').addEventListener('input', filterList);
        document.querySelectorAll('.pm-pill').forEach(function(pill){
            pill.addEventListener('click', function(){
                document.querySelectorAll('.pm-pill').forEach(function(p){p.classList.remove('on');});
                this.classList.add('on'); filterList();
            });
        });
        document.getElementById('tanggal_pinjam').addEventListener('change', function(){
            var d=new Date(this.value); d.setDate(d.getDate()+1);
            document.getElementById('tanggal_kembali').min=d.toISOString().split('T')[0]; calcDur();
        });
    ...
    function filterList() {
        ...
        var ap=document.querySelector('.pm-pill.on'), cat=ap?ap.getAttribute('data-cat'):'all';
        var rows=document.querySelectorAll('.pm-list-item'), vis=0;
        rows.forEach(function(r){
            var ok=(!kw||(r.getAttribute('data-nama')||'').toLowerCase().includes(kw))&&(cat==='all'||r.getAttribute('data-cat')===cat);
    ...
    function calcDur(){
        var p=document.getElementById('tanggal_pinjam').value, k=document.getElementById('tanggal_kembali').value;
        var box=document.getElementById('pmDurCard'), num=document.getElementById('pmDurNum');
        if(p&&k){var d=Math.ceil((new Date(k)-new Date(p))/86400000); if(d>0){num.textContent=d;box.style.display='block';return;}}
        num.textContent='–'; box.style.display='none';
    }
    </script>
